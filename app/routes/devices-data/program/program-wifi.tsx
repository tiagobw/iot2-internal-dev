...
import { useProgramWifi } from '~/routes/devices-data/program/use-program-wifi';
...
const formSchema = z.object({
  ...
  wifiSsid: z.string().min(1, 'Campo obrigatório'),
  wifiPassword: z.string().min(1, 'Campo obrigatório'),
});
...
export function ProgramWifi({ deviceId }: Props) {
  const [isPasswordVisible, setIsPasswordVisible] = useState(false);
  const { data, isLoading } = useProgramWifi(deviceId);
  const { executePost } = usePost('alx_prg_wifi');
  ...
  const isEnabled = form.watch('wifiEnabled');
      ...
      <Form {...form}>
        <form className='flex flex-col items-center w-full gap-6'>
          <div className='flex items-center gap-4'>
            <h3 className='font-bold'>Programar WiFi</h3>
            <AlertDialog
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex justify-between items-center w-full gap-2'>
                <FormLabel className='w-1/2'>Rede WiFi</FormLabel>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex justify-between items-center w-full gap-2'>
                <FormLabel className='w-1/2'>Senha WiFi</FormLabel>
